...
are stored in a special attribute named ``__rustree_dataclass_fields__`` in the dataclass.

>>> import math
... import rustree
...
>>> @rustree.dataclasses.dataclass(namespace='my_module')
... class Point:
...     x: float
...     y: float
...     z: float = 0.0
...     norm: float = rustree.dataclasses.field(init=False, pytree_node=False)
...
...
>>> rustree.tree_flatten(point)  # `Point`s are leaf nodes
([Point(x=2.0, y=6.0, z=3.0, norm=7.0)], PyTreeSpec(*))
>>> # Flatten with the namespace
>>> accessors, leaves, treespec = rustree.tree_flatten_with_accessor(point, namespace='my_module')
...
>>> point == rustree.tree_unflatten(treespec, leaves)
True
"""
...
from rustree.accessors import DataclassEntry
...
_FIELDS = '__rustree_dataclass_fields__'
