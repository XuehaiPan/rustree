...
@parametrize(
    tree=[
        rustree.functools.partial(dummy_func),
        rustree.functools.partial(dummy_func, 1, 2),
        rustree.functools.partial(dummy_func, x='a'),
        rustree.functools.partial(dummy_func, 1, 2, 3, x=4, y=5),
        rustree.functools.partial(dummy_func, 1, None, x=4, y=5, z=None),
        rustree.functools.partial(dummy_partial_func, 1, 2, 3, x=4, y=5),
    ],
...
def test_partial_does_not_merge_with_other_partials():
    ...
    h = rustree.functools.partial(g, 3)
    assert h.args == (3,)
...
def test_partial_func_attribute_has_stable_hash():
    ...
    p1 = rustree.functools.partial(fn, 2)
    p2 = rustree.functools.partial(fn, 2)
