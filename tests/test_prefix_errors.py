...
def test_different_types():
    lhs, rhs = (1, 2), [1, 2]
    lhs_treespec, rhs_treespec = rustree.tree_structure(lhs), rustree.tree_structure(rhs)
    with pytest.raises(ValueError, match=r'Expected an instance of tuple, got .*\.'):
        rustree.tree_map_(lambda x, y: None, lhs, rhs)
    assert not lhs_treespec.is_prefix(rhs_treespec)

    (e,) = rustree.prefix_errors(lhs, rhs)
    expected = re.escape(
        textwrap.dedent(
            """
...
def test_different_types_nested():
    ...
    lhs_treespec, rhs_treespec = (
        rustree.tree_structure(lhs, none_is_leaf=True),
        rustree.tree_structure(rhs, none_is_leaf=True),
    )
    rustree.tree_map_(lambda x, y: None, lhs, rhs, none_is_leaf=True)
    assert lhs_treespec.is_prefix(rhs_treespec)
    () = rustree.prefix_errors(lhs, rhs, none_is_leaf=True)
...
def test_different_types_multiple():
    ...
    e1, e2 = rustree.prefix_errors(lhs, rhs)
    expected = re.escape(
        textwrap.dedent(
            """
            pytree structure error: different types at key path
...
def test_different_metadata():
    ...
    lhs_treespec, rhs_treespec = rustree.tree_structure(lhs), rustree.tree_structure(rhs)
    rustree.tree_map_(lambda x, y: None, lhs, rhs)  # ignore key ordering
    assert lhs_treespec.is_prefix(rhs_treespec)
    () = rustree.prefix_errors(lhs, rhs)

    lhs, rhs = defaultdict(list, {'a': 1, 'b': [2, 3]}), defaultdict(set, {'b': [5, [6]], 'a': 4})
    lhs_treespec, rhs_treespec = rustree.tree_structure(lhs), rustree.tree_structure(rhs)
    rustree.tree_map_(lambda x, y: None, lhs, rhs)  # ignore default factory
    assert lhs_treespec.is_prefix(rhs_treespec)
    () = rustree.prefix_errors(lhs, rhs)

    lhs, rhs = {'a': 1, 'b': [2, 3]}, defaultdict(list, {'b': [5, [6]], 'a': 4})
    lhs_treespec, rhs_treespec = rustree.tree_structure(lhs), rustree.tree_structure(rhs)
    rustree.tree_map_(lambda x, y: None, lhs, rhs)  # ignore dictionary types
    assert lhs_treespec.is_prefix(rhs_treespec)
    () = rustree.prefix_errors(lhs, rhs)
    ...
    lhs, rhs = OrderedDict({'b': [2, 3], 'a': 1}), {'a': 4, 'b': [5, [6]]}
    lhs_treespec, rhs_treespec = rustree.tree_structure(lhs), rustree.tree_structure(rhs)
    rustree.tree_map_(lambda x, y: None, lhs, rhs)  # ignore dictionary types and key ordering
    assert lhs_treespec.is_prefix(rhs_treespec)
    () = rustree.prefix_errors(lhs, rhs)

    lhs, rhs = deque([1, 2], maxlen=None), deque([3, [4, 5]], maxlen=3)
    lhs_treespec, rhs_treespec = rustree.tree_structure(lhs), rustree.tree_structure(rhs)
    rustree.tree_map_(lambda x, y: None, lhs, rhs)  # ignore maxlen
    assert lhs_treespec.is_prefix(rhs_treespec)
    () = rustree.prefix_errors(lhs, rhs)
...
def test_standard_dictionary(
    ...
    with rustree.dict_insertion_ordered(
        ...
        shuffled_tree = rustree.tree_map(
            shuffle_dictionary,
            tree,
        ...
        shuffled_treespec = rustree.tree_structure(
            shuffled_tree,
            none_is_leaf=none_is_leaf,
            namespace=namespace,
        )
        shuffled_suffix_tree = rustree.tree_map(
            shuffle_dictionary,
            suffix_tree,
            is_leaf=lambda x: type(x) in {dict, OrderedDict, defaultdict},
            none_is_leaf=none_is_leaf,
        ...
        shuffled_suffix_treespec = rustree.tree_structure(
...
def test_fallback():
    (e,) = rustree.prefix_errors(Vector2D(1, [2]), Vector2D(3, 4))
    expected = re.escape(
        textwrap.dedent(
            """
            pytree structure error: different types at key path
