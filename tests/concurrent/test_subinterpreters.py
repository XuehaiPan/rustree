...
if (
    ...
    or not RUSTREE_HAS_SUBINTERPRETER_SUPPORT
):
...
def check_module_importable():
    ...
    is_current_interpreter_main = rustree._rs.is_current_interpreter_main()
    main_interpreter_id = rustree._rs.get_main_interpreter_id()
    current_interpreter_id = rustree._rs.get_current_interpreter_id()
    ...
    if not is_current_interpreter_main and rustree._rs.get_registry_size() != 8:
    ...
    leaves1, treespec1 = rustree.tree_flatten(tree, none_is_leaf=False)
    reconstructed1 = rustree.tree_unflatten(treespec1, leaves1)
    ...
    leaves2, treespec2 = rustree.tree_flatten(tree, none_is_leaf=True)
    reconstructed2 = rustree.tree_unflatten(treespec2, leaves2)
    ...
    _ = rustree.tree_flatten_with_path(tree, none_is_leaf=False)
    _ = rustree.tree_flatten_with_path(tree, none_is_leaf=True)
    _ = rustree.tree_flatten_with_accessor(tree, none_is_leaf=False)
    _ = rustree.tree_flatten_with_accessor(tree, none_is_leaf=True)
...
def test_import():
    ...
    for _ in range(random.randint(5, 10)):
        with contextlib.closing(interpreters.create()) as subinterpreter:
            subinterpreter.exec('import rustree')
        with contextlib.closing(interpreters.create()) as subinterpreter:
...
def test_import_in_subinterpreter_after_main():
    ...
    check_script_in_subprocess(
        ...
        del rustree, subinterpreter, subinterpreters, stack
        for _ in range(10):
            gc.collect()
        """,
        output='',
...
def test_import_in_subinterpreters_concurrently():
    check_script_in_subprocess(
        ...
        def check_import():
            ...
            if rustree._rs.get_registry_size() != 8:
                raise RuntimeError('registry size mismatch')
            if rustree._rs.is_current_interpreter_main():
