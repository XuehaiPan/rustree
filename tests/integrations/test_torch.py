...
def test_tree_ravel(tree):
    ...
    tree = rustree.tree_map(replace_leaf, tree)
    flat, unravel_func = rustree.integrations.torch.tree_ravel(tree)
    ...
    reconstructed_leaves, reconstructed_treespec = rustree.tree_flatten(reconstructed)
...
def test_tree_ravel_non_tensor():
    with pytest.raises(ValueError, match=r'All leaves must be tensors\.'):
        rustree.integrations.torch.tree_ravel(1)

    with pytest.raises(ValueError, match=r'All leaves must be tensors\.'):
        rustree.integrations.torch.tree_ravel((1, 2))

    with pytest.raises(ValueError, match=r'All leaves must be tensors\.'):
        rustree.integrations.torch.tree_ravel((torch.tensor(1), 2))

    rustree.integrations.torch.tree_ravel((torch.tensor(1), torch.tensor(2)))
