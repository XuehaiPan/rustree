...
def test_register_pytree_node_class_with_duplicate_namespace():
    with pytest.raises(
        ...
        @rustree.register_pytree_node_class('mylist', namespace='mylist')
        class MyList(UserList):
            def __tree_flatten__(self):
                return self.data, None, None
...
def test_register_pytree_node_with_non_class():
    ...
    with pytest.raises(TypeError, match='Expected a class'):
        ...
        @rustree.register_pytree_node_class(namespace='func')
        def func2():
            pass
...
def test_register_pytree_node_class_with_duplicate_registrations():
    @rustree.register_pytree_node_class('mylist1')
    ...
    @rustree.register_pytree_node_class(namespace='mylist2')
    ...
    with pytest.raises(
        ...
        rustree.register_pytree_node_class(MyList1, namespace='mylist1')
    with pytest.raises(
        ValueError,
        match=r"PyTree type.*is already registered in namespace 'mylist2'\.",
    ):
        rustree.register_pytree_node_class(MyList2, namespace='mylist2')

    rustree.register_pytree_node_class(namespace='mylist3')(MyList1)


def test_register_pytree_node_with_invalid_namespace():
    with pytest.raises(TypeError, match='The namespace must be a string'):

        @rustree.register_pytree_node_class(namespace=1)
        class MyList1(UserList):
            def __tree_flatten__(self):
                return self.data, None, None
...
def test_register_pytree_node_with_invalid_path_entry_type():
    with pytest.raises(TypeError, match=r'Expected a subclass of PyTreeEntry, got .*\.'):
        ...
        @rustree.register_pytree_node_class(namespace='error')
        class MyList1(UserList):
    ...
    with pytest.raises(TypeError, match=r'Expected a subclass of PyTreeEntry, got .*\.'):
        ...
        @rustree.register_pytree_node_class(path_entry_type=1, namespace='error')
        class MyList3(UserList):
            def __tree_flatten__(self):
                return self.data, None, None
    ...
    with pytest.raises(TypeError, match=r'Expected a subclass of PyTreeEntry, got .*\.'):
        ...
        @rustree.register_pytree_node_class(path_entry_type=int, namespace='error')
...
def test_register_pytree_node_namedtuple():
    ...
    tree = mytuple1(1, 2, 3)
    leaves1, treespec1 = rustree.tree_flatten(tree)
    assert leaves1 == [3, 2, 1]
    assert str(treespec1) == 'PyTreeSpec(CustomTreeNode(mytuple1[None], [*, *, *]))'
    assert tree == rustree.tree_unflatten(treespec1, leaves1)

    leaves2, treespec2 = rustree.tree_flatten(tree, namespace='undefined')
    assert leaves2 == [3, 2, 1]
    assert (
        str(treespec2)
        == "PyTreeSpec(CustomTreeNode(mytuple1[None], [*, *, *]), namespace='undefined')"
    )
    assert tree == rustree.tree_unflatten(treespec2, leaves2)
    assert treespec1 == treespec2

    leaves3, treespec3 = rustree.tree_flatten(tree, namespace='mytuple')
    assert leaves3 == [2, 3, 1]
    assert (
        str(treespec3)
        == "PyTreeSpec(CustomTreeNode(mytuple1[None], [*, *, *]), namespace='mytuple')"
    )
    assert tree == rustree.tree_unflatten(treespec3, leaves3)
...
def test_flatten_with_wrong_number_of_returns():
    ...
    with pytest.raises(
        ...
        rustree.tree_flatten(MyList1([1, 2, 3]), namespace='error')

    with pytest.raises(
        RuntimeError,
        match=r"PyTree custom flatten function for type <class '.*'> should return a 2- or 3-tuple, got 1\.",
    ):
        rustree.tree_flatten_one_level(MyList1([1, 2, 3]), namespace='error')
    ...
    with pytest.raises(
        ...
        rustree.tree_flatten(MyList4([1, 2, 3]), namespace='error')

    with pytest.raises(
        RuntimeError,
        match=r"PyTree custom flatten function for type <class '.*'> should return a 2- or 3-tuple, got 4\.",
    ):
        rustree.tree_flatten_one_level(MyList4([1, 2, 3]), namespace='error')
    ...
    with pytest.raises(
        ...
        rustree.tree_flatten(MyListEntryMismatch([1, 2, 3]), namespace='error')

    with pytest.raises(
        RuntimeError,
        match=r"PyTree custom flatten function for type <class '.*'> returned inconsistent number of children \(3\) and number of entries \(4\)\.",
    ):
        rustree.tree_flatten_one_level(MyListEntryMismatch([1, 2, 3]), namespace='error')


def test_pytree_node_registry_get():
    registry1 = rustree.register_pytree_node.get()
    assert rustree.register_pytree_node.get() == registry1
    assert rustree.register_pytree_node.get(namespace=GLOBAL_NAMESPACE) == registry1
    assert rustree.register_pytree_node.get(namespace='undefined') == registry1
    assert rustree.register_pytree_node.get() is not registry1
    assert all(handler.namespace == '' for handler in registry1.values())
    with rustree.dict_insertion_ordered(True, namespace=GLOBAL_NAMESPACE):
        registry2 = rustree.register_pytree_node.get()
        assert registry2 != registry1
        assert list(registry2) == list(registry1)
        assert rustree.register_pytree_node.get() == registry2
        assert rustree.register_pytree_node.get(namespace='undefined') == registry2
        assert rustree.register_pytree_node.get() is not registry2
        assert all(handler.namespace == '' for handler in registry2.values())

    handler = rustree.register_pytree_node.get(list)
    ...
    handler = rustree.register_pytree_node.get(list, namespace='any')
    assert handler is not None
    assert handler is registry1[list]
    assert handler is registry2[list]
    lst = [1, 2, 3]
    ...
    handler = rustree.register_pytree_node.get(dict)
    assert handler is not None
    ...
    handler = rustree.register_pytree_node.get(dict, namespace='any')
    assert handler is not None
    assert handler is registry1[dict]
    assert handler is not registry2[dict]
    dct = {'b': 2, 'c': 3, 'a': 1}
    ...
    handler = rustree.register_pytree_node.get(set)
    ...
    registry3 = rustree.register_pytree_node.get()
    handler = rustree.register_pytree_node.get(set)
    assert handler is not None
    ...
    handler = rustree.register_pytree_node.get(set, namespace='set')
    assert handler is not None
    assert rustree.register_pytree_node.get(namespace='set') == registry3
    ...
    handler = rustree.register_pytree_node.get(MyList)
    assert handler is None
    assert rustree.register_pytree_node.get() == registry3
    assert MyList not in registry3
    handler = rustree.register_pytree_node.get(MyList, namespace='set')
    assert handler is None
    assert MyList not in registry3

    handler = rustree.register_pytree_node.get(MyList, namespace='mylist')
    registry4 = rustree.register_pytree_node.get(namespace='mylist')
    assert handler is not None
...
def test_pytree_node_registry_get_with_invalid_arguments():
    registry = rustree.register_pytree_node.get()
    assert rustree.register_pytree_node.get(None) == registry
    assert rustree.register_pytree_node.get(namespace=GLOBAL_NAMESPACE) == registry
    assert rustree.register_pytree_node.get(namedtuple) is registry[namedtuple]  # noqa: PYI024
    if sys.version_info[:2] != (3, 9) or PYPY:
        with pytest.raises(TypeError, match='Expected a class or None'):
            rustree.register_pytree_node.get(dataclass)
        with pytest.raises(TypeError, match='The namespace must be a string'):
            rustree.register_pytree_node.get(list, namespace=None)


def test_pytree_node_registry_with_init_subclass():
    @rustree.register_pytree_node_class(namespace='mydict')
    class MyDict(UserDict):
        def __init_subclass__(cls):
            super().__init_subclass__()
            rustree.register_pytree_node_class(cls, namespace='mydict')
    ...
    leaves, treespec = rustree.tree_flatten(tree, namespace='mydict')
    assert leaves == [6, 5, 4, 2, 3]
    assert (
        str(treespec)
        == "PyTreeSpec(CustomTreeNode(MyDict[['c', 'b', 'a']], [CustomTreeNode(MyAnotherDict[['f', 'd']], [*, *]), *, (*, *)]), namespace='mydict')"
    )
    paths, leaves, treespec = rustree.tree_flatten_with_path(tree, namespace='mydict')
    ...
    accessors, leaves, treespec = rustree.tree_flatten_with_accessor(tree, namespace='mydict')
    assert accessors == [
        rustree.PyTreeAccessor(
            (
                rustree.MappingEntry('c', MyDict, rustree.PyTreeKind.CUSTOM),
                rustree.MappingEntry('f', MyAnotherDict, rustree.PyTreeKind.CUSTOM),
            ),
        ),
        rustree.PyTreeAccessor(
            (
                rustree.MappingEntry('c', MyDict, rustree.PyTreeKind.CUSTOM),
                rustree.MappingEntry('d', MyAnotherDict, rustree.PyTreeKind.CUSTOM),
            ),
        ),
        rustree.PyTreeAccessor((rustree.MappingEntry('b', MyDict, rustree.PyTreeKind.CUSTOM),)),
        rustree.PyTreeAccessor(
            (
...
def test_unregister_pytree_node_with_non_class():
    with pytest.raises(TypeError, match='Expected a class'):
        rustree.unregister_pytree_node(dataclass, namespace=GLOBAL_NAMESPACE)

    def func1():
        pass

    with pytest.raises(TypeError, match='Expected a class'):
        rustree.unregister_pytree_node(func1, namespace=GLOBAL_NAMESPACE)

    with pytest.raises(TypeError, match='Expected a class'):
        rustree.unregister_pytree_node(1, namespace=GLOBAL_NAMESPACE)

    def func2():
        pass

    with pytest.raises(TypeError, match='Expected a class'):
        rustree.unregister_pytree_node(func2, namespace='func')

    with pytest.raises(TypeError, match='Expected a class'):
        rustree.unregister_pytree_node(1, namespace='non-class')
...
def test_unregister_pytree_node_with_non_registered_class():
    ...
    with pytest.raises(
        ...
        rustree.unregister_pytree_node(MyList, namespace='undefined')

    with pytest.raises(
        ValueError,
        match=r"PyTree type <class '.*'> is not registered in the global namespace\.",
    ):
        rustree.unregister_pytree_node(MyList, namespace=GLOBAL_NAMESPACE)
    ...
    rustree.unregister_pytree_node(MyList, namespace='mylist')
...
def test_unregister_pytree_node_with_invalid_namespace():
    with pytest.raises(TypeError, match='The namespace must be a string'):
        rustree.unregister_pytree_node(set, namespace=1)

    with pytest.raises(ValueError, match=re.escape('The namespace cannot be an empty string.')):
        rustree.unregister_pytree_node(set, namespace='')
...
def test_unregister_pytree_node_with_builtins():
    with pytest.raises(
        ...
        rustree.unregister_pytree_node(type(None), namespace=GLOBAL_NAMESPACE)
    ...
    with pytest.raises(
        ...
        rustree.unregister_pytree_node(type(None), namespace='none')
    ...
    with pytest.raises(
        ...
        rustree.unregister_pytree_node(list, namespace=GLOBAL_NAMESPACE)
    ...
    with pytest.raises(
        ...
        rustree.unregister_pytree_node(list, namespace='list')
...
def test_unregister_pytree_node_namedtuple():
    ...
    rustree.unregister_pytree_node(mytuple1, namespace=GLOBAL_NAMESPACE)
    assert str(treespec1) == 'PyTreeSpec(CustomTreeNode(mytuple1[None], [*, *, *]))'
    assert tree == rustree.tree_unflatten(treespec1, leaves1)

    leaves2, treespec2 = rustree.tree_flatten(tree)
    assert leaves2 == [1, 2, 3]
    ...
    with pytest.raises(
        ...
        rustree.unregister_pytree_node(mytuple1, namespace='undefined')
    with pytest.raises(
        ValueError,
        match=re.escape(
            r"PyTree type <class 'test_registry.mytuple1'> is a subclass of `collections.namedtuple`, "
    ...
    rustree.unregister_pytree_node(mytuple2, namespace='mytuple')
    assert (
        str(treespec3)
        == "PyTreeSpec(CustomTreeNode(mytuple2[None], [*, *, *]), namespace='mytuple')"
    )
    ...
    leaves4, treespec4 = rustree.tree_flatten(tree, namespace='mytuple')
    assert leaves4 == [1, 2, 3]
    assert str(treespec4) == 'PyTreeSpec(mytuple2(a=*, b=*, c=*))'
    assert tree == rustree.tree_unflatten(treespec4, leaves4)
...
def test_unregister_pytree_node_no_reference_leak():  # noqa: C901
    ...
    rustree.unregister_pytree_node(MyList1, namespace=GLOBAL_NAMESPACE)
    del MyList1
    gc_collect()
    ...
    leaves, treespec = rustree.tree_flatten(MyList2([1, 2, 3]))
    assert leaves == [3, 2, 1]
    assert str(treespec) == 'PyTreeSpec(CustomTreeNode(MyList2[None], [*, *, *]))'

    rustree.unregister_pytree_node(MyList2, namespace=GLOBAL_NAMESPACE)
    del MyList2
    gc_collect()
    assert wr() is not None
    assert wr() is treespec.type
    assert rustree.tree_unflatten(treespec, leaves) == wr()([1, 2, 3])
    ...
    leaves, treespec = rustree.tree_flatten(MyList3([1, 2, 3]), namespace='undefined')
    assert leaves == [3, 2, 1]
    assert (
        str(treespec)
    ...
    rustree.unregister_pytree_node(MyList3, namespace=GLOBAL_NAMESPACE)
    del MyList3
    gc_collect()
    assert wr() is not None
    assert wr() is treespec.type
    ...
    rustree.unregister_pytree_node(MyList4, namespace='mylist')
    del MyList4
    gc_collect()
    if not Py_GIL_DISABLED:
        assert wr() is None
    ...
    leaves, treespec = rustree.tree_flatten(MyList5([1, 2, 3]), namespace='mylist')
    assert leaves == [3, 2, 1]
    assert (
        str(treespec) == "PyTreeSpec(CustomTreeNode(MyList5[None], [*, *, *]), namespace='mylist')"
    )

    rustree.unregister_pytree_node(MyList5, namespace='mylist')
    del MyList5
    gc_collect()
    assert wr() is not None
    assert wr() is treespec.type
...
def test_dict_insertion_order_with_invalid_namespace():
    with pytest.raises(TypeError, match='The namespace must be a string'):
        with rustree.dict_insertion_ordered(True, namespace=1):
            pass
    with pytest.raises(ValueError, match=re.escape('The namespace cannot be an empty string.')):
        with rustree.dict_insertion_ordered(True, namespace=''):
            pass


def test_dict_insertion_order_with_nested_context():
    def is_dict_insertion_ordered(namespace):
        insertion_ordered = rustree._rs.is_dict_insertion_ordered(namespace)
        assert rustree.tree_leaves({'b': 2, 'a': 1}, namespace=namespace) == (
    ...
    with rustree.dict_insertion_ordered(True, namespace=GLOBAL_NAMESPACE):
        ...
        with rustree.dict_insertion_ordered(True, namespace='namespace'):
            assert is_dict_insertion_ordered('')
            assert is_dict_insertion_ordered('namespace')
            assert is_dict_insertion_ordered('other-namespace')
            with rustree.dict_insertion_ordered(False, namespace=GLOBAL_NAMESPACE):
                assert not is_dict_insertion_ordered('')
                assert is_dict_insertion_ordered('namespace')
                assert not is_dict_insertion_ordered('other-namespace')

                with rustree.dict_insertion_ordered(True, namespace='other-namespace'):
                    assert not is_dict_insertion_ordered('')
                    assert is_dict_insertion_ordered('namespace')
                    assert is_dict_insertion_ordered('other-namespace')
        ...
        assert is_dict_insertion_ordered('namespace')
        assert is_dict_insertion_ordered('other-namespace')
        with rustree.dict_insertion_ordered(False, namespace='namespace'):
            assert is_dict_insertion_ordered('')
            assert is_dict_insertion_ordered('namespace')
            assert is_dict_insertion_ordered('other-namespace')
            with rustree.dict_insertion_ordered(False, namespace=GLOBAL_NAMESPACE):
    ...
    with rustree.dict_insertion_ordered(True, namespace='namespace'):
        ...
        with rustree.dict_insertion_ordered(True, namespace='other-namespace'):
            ...
            with rustree.dict_insertion_ordered(False, namespace='other-namespace'):
                assert not is_dict_insertion_ordered('')
                assert is_dict_insertion_ordered('namespace')
                assert not is_dict_insertion_ordered('other-namespace')
...
def test_register_pytree_node_class_legacy_methods():
    @rustree.register_pytree_node_class(namespace='legacy')
    ...
    leaves, treespec = rustree.tree_flatten(tree, namespace='legacy')
    assert leaves == [1, 2, 3]
    assert tree == rustree.tree_unflatten(treespec, leaves)

    @rustree.register_pytree_node_class(namespace='mixed')
    ...
    leaves, treespec = rustree.tree_flatten(tree, namespace='mixed')
    assert leaves == [1, 2, 3]
    assert tree == rustree.tree_unflatten(treespec, leaves)
...
def test_register_pytree_node_class_wrapping_behavior():
    @rustree.register_pytree_node_class(namespace='wrapping')
    ...
    leaves, treespec = rustree.tree_flatten(tree, namespace='wrapping')
    assert leaves == [3, 2, 1]
    assert tree == rustree.tree_unflatten(treespec, leaves)
...
def test_registry_size():
    initial_size = len(rustree.register_pytree_node.get())
    initial_total_size = len(NODETYPE_REGISTRY)
    initial_registry_size = len(rustree.register_pytree_node.get(namespace='mylist'))
    assert rustree._rs.get_registry_size() + 2 == initial_total_size
    assert rustree._rs.get_registry_size(namespace='') + 2 == initial_size
    assert rustree._rs.get_registry_size(namespace='mylist') + 2 == initial_registry_size
    assert rustree._rs.get_registry_size(namespace='undefined') + 2 == initial_size
    assert len(rustree.register_pytree_node.get(namespace='undefined')) == initial_size
    ...
    assert len(rustree.register_pytree_node.get()) == initial_size
    assert len(NODETYPE_REGISTRY) == initial_total_size + 1
    assert len(rustree.register_pytree_node.get(namespace='mylist')) == initial_registry_size + 1
    assert rustree._rs.get_registry_size() + 2 == initial_total_size + 1
    assert rustree._rs.get_registry_size(namespace='') + 2 == initial_size
    assert rustree._rs.get_registry_size(namespace='mylist') + 2 == initial_registry_size + 1
    assert rustree._rs.get_registry_size(namespace='undefined') + 2 == initial_size
    assert len(rustree.register_pytree_node.get(namespace='undefined')) == initial_size
    ...
    assert len(rustree.register_pytree_node.get(namespace='mylist')) == initial_registry_size
    assert rustree._rs.get_registry_size() + 2 == initial_total_size
    assert rustree._rs.get_registry_size(namespace='') + 2 == initial_size
    assert rustree._rs.get_registry_size(namespace='mylist') + 2 == initial_registry_size
    assert rustree._rs.get_registry_size(namespace='undefined') + 2 == initial_size
