...
def test_pytreekind_enum():
    if RUSTREE_HAS_NATIVE_ENUM:
        all_kinds = list(rustree.PyTreeKind)
        assert len(all_kinds) == rustree.PyTreeKind.NUM_KINDS
        assert issubclass(rustree.PyTreeKind, enum.IntEnum)
        assert issubclass(rustree.PyTreeKind, int)

        assert rustree.PyTreeKind.CUSTOM == 0
        assert rustree.PyTreeKind.LEAF == 1
        assert rustree.PyTreeKind.NONE == 2
        assert rustree.PyTreeKind.CUSTOM.name == 'CUSTOM'
        assert rustree.PyTreeKind.LEAF.name == 'LEAF'
        assert rustree.PyTreeKind.NONE.name == 'NONE'
        for i, kind in enumerate(all_kinds):
            assert isinstance(kind, int)
            assert kind == i
            assert kind is rustree.PyTreeKind(i)
            assert kind is getattr(rustree.PyTreeKind, kind.name)

        with pytest.raises(ValueError, match=r'.* is not a valid .*\bPyTreeKind\b.*'):
            rustree.PyTreeKind(rustree.PyTreeKind.NUM_KINDS)
    else:
        all_kinds = [rustree.PyTreeKind(i) for i in range(rustree.PyTreeKind.NUM_KINDS)]

    assert rustree.PyTreeKind.CUSTOM.value == 0
    assert rustree.PyTreeKind.LEAF.value == 1
    assert rustree.PyTreeKind.NONE.value == 2
    assert rustree.PyTreeKind.CUSTOM.name == 'CUSTOM'
    assert rustree.PyTreeKind.LEAF.name == 'LEAF'
    assert rustree.PyTreeKind.NONE.name == 'NONE'
    for i, kind in enumerate(all_kinds):
        assert isinstance(kind, rustree.PyTreeKind)
        assert int(kind) == i
        assert kind.value == i
        assert kind == rustree.PyTreeKind(i)
        assert kind == getattr(rustree.PyTreeKind, kind.name)
...
def test_pytree_typing():
    ...
    rustree.PyTree[int]
    rustree.PyTree[Union[int, str]]
    rustree.PyTree[T]
    assert rustree.PyTree[rustree.PyTree[int]] == rustree.PyTree[int]
    assert rustree.PyTree[rustree.PyTree[Union[int, str]]] == rustree.PyTree[Union[int, str]]
    assert rustree.PyTree[rustree.PyTree[T]] == rustree.PyTree[T]
    if sys.version_info >= (3, 10):
        rustree.PyTree[int | str]
        assert rustree.PyTree[rustree.PyTree[int | str]] == rustree.PyTree[int | str]

    IntTree = rustree.PyTreeTypeVar('IntTree', int)  # noqa: N806
    assert IntTree == rustree.PyTree[IntTree]
...
def test_is_namedtuple():
    ...
    for is_namedtuple, is_namedtuple_class, is_namedtuple_instance in (  # noqa: B007
        (
            rustree.is_namedtuple,
            rustree.is_namedtuple_class,
            rustree.is_namedtuple_instance,
        ),
        (
            rustree.is_namedtuple.__rust_implementation__,
            rustree.is_namedtuple_class.__rust_implementation__,
            rustree.is_namedtuple_instance.__rust_implementation__,
        ),
        (
            rustree.is_namedtuple.__python_implementation__,
            rustree.is_namedtuple_class.__python_implementation__,
            rustree.is_namedtuple_instance.__python_implementation__,
        ),
    ):
...
def test_is_namedtuple_cache():
    ...
    assert rustree.is_namedtuple(Point)
    ...
    assert rustree.is_namedtuple_class(Point)
    ...
    assert not rustree.is_namedtuple(time.struct_time)
    new_refcount = getrefcount(time.struct_time)
    new_weakrefcount = weakref.getweakrefcount(time.struct_time)
    assert new_refcount == refcount
    assert new_weakrefcount <= weakrefcount + 1
    assert not rustree.is_namedtuple_class(time.struct_time)
    ...
    assert not rustree.is_namedtuple(Foo)
    new_refcount = getrefcount(Foo)
    new_weakrefcount = weakref.getweakrefcount(Foo)
    assert new_refcount == refcount
    assert new_weakrefcount == weakrefcount + 1
    assert not rustree.is_namedtuple_class(Foo)
...
def test_namedtuple_fields():
    for namedtuple_fields in (
        rustree.namedtuple_fields,
        rustree.namedtuple_fields.__rust_implementation__,
        rustree.namedtuple_fields.__python_implementation__,
    ):
...
def test_namedtuple_fields_cache():
    ...
    refcount = getrefcount(Point)
    weakrefcount = weakref.getweakrefcount(Point)
    assert rustree.namedtuple_fields(Point) == ('x', 'y')
    new_refcount = getrefcount(Point)
    new_weakrefcount = weakref.getweakrefcount(Point)
    assert new_refcount == refcount
    assert new_weakrefcount == weakrefcount + 1
    assert rustree.namedtuple_fields(Point(0, 1)) == ('x', 'y')
    assert weakref.getweakrefcount(Point) == new_weakrefcount
    wr = weakref.getweakrefs(Point)[0]
    assert wr() is Point

    fields = rustree.namedtuple_fields(Point)
    assert rustree.namedtuple_fields(Point) is fields
    assert rustree.namedtuple_fields(Point(0, 1)) is fields
    new_fields = ('a', 'b')
    Point._fields = new_fields
    assert rustree.namedtuple_fields(Point) is new_fields
    assert rustree.namedtuple_fields(Point(0, 1)) is new_fields
    ...
    with pytest.raises(
        ...
        assert rustree.namedtuple_fields(time.struct_time)
...
def test_is_structseq():
    ...
    with pytest.raises(TypeError, match="type 'StructSequence' is not an acceptable base type"):
        ...
        class MyTuple(rustree.typing.StructSequence):
            pass

    with pytest.raises(NotImplementedError):
        rustree.typing.StructSequence(range(1))

    for is_structseq, is_structseq_class, is_structseq_instance in (  # noqa: B007
        (
            rustree.is_structseq,
            rustree.is_structseq_class,
            rustree.is_structseq_instance,
        ),
        (
            rustree.is_structseq.__rust_implementation__,
            rustree.is_structseq_class.__rust_implementation__,
            rustree.is_structseq_instance.__rust_implementation__,
        ),
        (
            rustree.is_structseq.__python_implementation__,
            rustree.is_structseq_class.__python_implementation__,
            rustree.is_structseq_instance.__python_implementation__,
        ),
    ):
...
def test_is_structseq_cache():
    ...
    refcount = getrefcount(Point)
    weakrefcount = weakref.getweakrefcount(Point)
    assert not rustree.is_structseq(Point)
    new_refcount = getrefcount(Point)
    new_weakrefcount = weakref.getweakrefcount(Point)
    assert new_refcount == refcount
    assert new_weakrefcount == weakrefcount + 1
    assert not rustree.is_structseq_class(Point)
    assert weakref.getweakrefcount(Point) == new_weakrefcount
    wr = weakref.getweakrefs(Point)[0]
    assert wr() is Point
    del Point
    ...
    refcount = getrefcount(time.struct_time)
    weakrefcount = weakref.getweakrefcount(time.struct_time)
    assert rustree.is_structseq(time.struct_time)
    new_refcount = getrefcount(time.struct_time)
    new_weakrefcount = weakref.getweakrefcount(time.struct_time)
    assert new_refcount == refcount
    assert new_weakrefcount <= weakrefcount + 1
    assert rustree.is_structseq_class(time.struct_time)
    assert weakref.getweakrefcount(time.struct_time) == new_weakrefcount
    ...
    refcount = getrefcount(Foo)
    weakrefcount = weakref.getweakrefcount(Foo)
    assert not rustree.is_structseq(Foo)
    new_refcount = getrefcount(Foo)
    new_weakrefcount = weakref.getweakrefcount(Foo)
    assert new_refcount == refcount
    assert new_weakrefcount == weakrefcount + 1
    assert not rustree.is_structseq_class(Foo)
    assert weakref.getweakrefcount(Foo) == new_weakrefcount
...
def test_structseq_fields():
    for structseq_fields in (
        rustree.structseq_fields,
        rustree.structseq_fields.__rust_implementation__,
        rustree.structseq_fields.__python_implementation__,
    ):
...
def test_structseq_fields_cache():
    ...
    with pytest.raises(TypeError, match=r"Expected a PyStructSequence type, got <class '.*'>\."):
        rustree.structseq_fields(Point)
    new_refcount = getrefcount(Point)
    new_weakrefcount = weakref.getweakrefcount(Point)
    assert new_refcount == refcount
    assert new_weakrefcount == weakrefcount + 1
    with pytest.raises(
        ...
        rustree.structseq_fields(Point(0, 1))
    assert weakref.getweakrefcount(Point) == new_weakrefcount
    wr = weakref.getweakrefs(Point)[0]
    assert wr() is Point
    del Point
    ...
    refcount = getrefcount(time.struct_time)
    weakrefcount = weakref.getweakrefcount(time.struct_time)
    assert rustree.structseq_fields(time.struct_time) is rustree.structseq_fields(time.struct_time)
    new_refcount = getrefcount(time.struct_time)
    new_weakrefcount = weakref.getweakrefcount(time.struct_time)
    assert new_refcount == refcount
    assert new_weakrefcount <= weakrefcount + 2
    assert rustree.structseq_fields(time.gmtime()) is rustree.structseq_fields(time.struct_time)
    assert weakref.getweakrefcount(time.struct_time) == new_weakrefcount
